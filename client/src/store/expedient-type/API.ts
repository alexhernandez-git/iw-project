...
export function fetchExpedientType(id: string) {
  return new Promise<{
    ...
    resolve(
      axios.get(`http://34.242.52.229:8080/expedient-types/${id}`, {
        headers: {
...
export function createExpedientType(data) {
  return new Promise<{ data: ExpedientType }>((resolve) =>
    resolve(
      axios.post("http://34.242.52.229:8080/expedient-types", data, {
        headers: {
          Authorization: `Bearer ${window.localStorage.getItem("token")}`,
        },
      })
...
export function updateExpedientType(id, data) {
  return new Promise<{ data: ExpedientType }>((resolve) =>
    resolve(
      axios.patch(`http://34.242.52.229:8080/expedient-types/${id}`, data, {
        headers: {
          Authorization: `Bearer ${window.localStorage.getItem("token")}`,
        },
      })
