...
export function fetchExpedientType(id: string) {
  return new Promise<{
    ...
    resolve(
      axios.get(`http://52.214.212.142/api/expedient-types/${id}`, {
        headers: {
...
export function createExpedientType(data) {
  return new Promise<{ data: ExpedientType }>((resolve) =>
    resolve(
      axios.post(`http://52.214.212.142/api/expedient-types`, data, {
        headers: {
          Authorization: `Bearer ${window.localStorage.getItem(`token`)}`,
        },
      })
...
export function updateExpedientType(id, data) {
  return new Promise<{ data: ExpedientType }>((resolve) =>
    resolve(
      axios.patch(`http://52.214.212.142/api/expedient-types/${id}/`, data, {
        headers: {
          Authorization: `Bearer ${window.localStorage.getItem(`token`)}`,
...
export function updateFileExpedientType(id, data) {
  return new Promise<{ data: { expedientType: ExpedientType } }>((resolve) =>
    resolve(
      axios.patch(
        `http://52.214.212.142/api/expedient-types/files/${id}`,
        data,
        {
          headers: {
            Authorization: `Bearer ${window.localStorage.getItem("token")}`,
...
export function deleteExpedientType(id) {
  return new Promise((resolve) =>
    resolve(
      axios.delete(`http://52.214.212.142/api/expedient-types/${id}`, {
        headers: {
          Authorization: `Bearer ${window.localStorage.getItem("token")}`,
          "Content-Type": "multipart/form-data",
        },
