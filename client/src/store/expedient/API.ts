...
export function fetchExpedient(id: string) {
  return new Promise<{ data: Expedient }>((resolve) =>
    resolve(
      axios.get(`http://localhost:8000/api/expedients/${id}`, {
        headers: {
...
export function updateExpedient(id, data) {
  return new Promise<{ data: any }>((resolve) =>
    resolve(
      axios.patch(`http://localhost:8000/api/expedients/${id}/`, data, {
        headers: {
          Authorization: `Bearer ${window.localStorage.getItem("token")}`,
...
export function updateFileExpedient(id, data) {
  return new Promise<{ data: any }>((resolve) =>
    resolve(
      axios.patch(`http://localhost:8000/api/expedients/files/${id}`, data, {
        headers: {
          Authorization: `Bearer ${window.localStorage.getItem("token")}`,
          "Content-Type": "multipart/form-data",
        },
...
export function deleteExpedient(id) {
  return new Promise((resolve) =>
    resolve(
      axios.delete(`http://localhost:8000/api/expedients/${id}`, {
        headers: {
          Authorization: `Bearer ${window.localStorage.getItem("token")}`,
          "Content-Type": "multipart/form-data",
        },
