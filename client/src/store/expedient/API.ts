...
export function fetchExpedient(id: string) {
  return new Promise<{ data: Expedient }>((resolve) =>
    resolve(
      axios.get(`http://52.214.212.142/api/expedients/${id}`, {
        headers: {
...
export function updateExpedient(id, data) {
  return new Promise<{ data: any }>((resolve) =>
    resolve(
      axios.patch(`http://52.214.212.142/api/expedients/${id}/`, data, {
        headers: {
          Authorization: `Bearer ${window.localStorage.getItem("token")}`,
...
export function updateFileExpedient(id, data) {
  return new Promise<{ data: any }>((resolve) =>
    resolve(
      axios.patch(`http://52.214.212.142/api/expedients/files/${id}`, data, {
        headers: {
          Authorization: `Bearer ${window.localStorage.getItem("token")}`,
          "Content-Type": "multipart/form-data",
        },
...
export function deleteExpedient(id) {
  return new Promise((resolve) =>
    resolve(
      axios.delete(`http://52.214.212.142/api/expedients/${id}`, {
        headers: {
          Authorization: `Bearer ${window.localStorage.getItem("token")}`,
          "Content-Type": "multipart/form-data",
        },
